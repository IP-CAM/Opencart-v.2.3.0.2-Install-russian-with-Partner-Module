...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			if ($this->user->hasPermission('access', 'module/partner')) {
				$partner[] = array(
					'name'	   => $this->language->get('text_partner'),
					'href'     => $this->url->link('module/partner', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);
			}
			
			if ($this->user->hasPermission('access', 'module/taxes')) {
				$partner[] = array(
					'name'	   => $this->language->get('text_taxes'),
					'href'     => $this->url->link('module/taxes', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);
			}
